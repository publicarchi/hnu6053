        ...
        <html lang="fr">
            ...
            <head>
                ...
                <title>
                    ...
                    <xsl:if test="/TEI/teiHeader/*/titleStmt/title[@type='subtitle']">
                        <xsl:text>: </xsl:text>
                        <xsl:value-of
                            select="normalize-space( /TEI/teiHeader/*/titleStmt/title[@type='subtitle'] )" />
                        <!-- utilisation d'une fonction pour normaliser les espaces -->
                        <!--<xsl:value-of select="concat( ': ', normalize-space(/TEI/teiHeader/*/titleStmt/title/@subtitle) )"/><!-\- utilisation d'une fonction pour concaténer -\->-->
                    </xsl:if>
                </title>
            ...
            <body id="feed">
                ...
                <nav class="ui inverted vertical sidebar menu">
                    ...
                    <xsl:apply-templates select="/TEI/text/group/text" mode="tdm" />
...
<!-- TODO traiter le front (ex003) -->
    <xsl:template match="front">
        <header id="front">
            ...
            <div class="ui horizontal segment">
                <xsl:apply-templates select="argument | div[@type='tradition']" />
            </div>
        </header>
    ...
    <xsl:template match="group/text" mode="tdm">
        ...
        <a class="item" href="#{@xml:id}">
            <xsl:apply-templates select="front/head" />
            <!-- définition de l'ordre de parcours -->
            <xsl:text>, </xsl:text>
            <!-- générer du texte -->
            <xsl:apply-templates select="front/docDate/placeName" />
            <!-- définition de l'ordre de parcours -->
            <xsl:text>, </xsl:text>
            <xsl:apply-templates select="front/docDate/date" />
            <!-- définition de l'ordre de parcours -->
        </a>
    </xsl:template>
    ...
    <xsl:template match="ref">
        <a href="{@target}">
            <!-- puisque les ancres sont déjà prêtes -->
            <xsl:attribute name="id">
                <xsl:text>note</xsl:text>
                <xsl:number count="ref" from="text" />
                <!-- générer un numéro depuis l'élément text -->
            </xsl:attribute>
            <xsl:number count="ref" from="text" />
        </a>
    </xsl:template>
